...
export default function ProjectDetailScreen() {
  ...
  useEffect(() => {
    fetchProject();
    fetchReceiptImages();
  }, [fetchProject, fetchReceiptImages]);
  ...
  const handleSend = () => {
    router.push({
      pathname: '/(app)/disclaimer',
      params: { projectId: id, action: 'send' }
    });
  };
  ...
  const handleTakePhoto = async (cameraRef: CameraView | null) => {
    if (cameraRef) {
      try {
  ...
  // FEATURE 3: Send Notice via Email
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {receiptImages.length > 0 ? (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.receiptScroll}>
              {receiptImages.map((uri, index) => (
                <Image key={index} source={{ uri }} style={styles.receiptImage} alt={`Receipt ${index + 1}`} />
              ))}
            </ScrollView>
          ) : (
        ...
        {project.notice_required && (
          <View style={styles.pdfActionsCard}>
            ...
            <View style={styles.pdfActionsRow}>
              <TouchableOpacity style={styles.pdfActionButton} onPress={handleQuickShare} disabled={pdfLoading}>
              ...
              <TouchableOpacity style={styles.pdfActionButton} onPress={handleQuickPrint} disabled={pdfLoading}>
                <View style={styles.pdfActionIcon}>
              ...
              <TouchableOpacity style={styles.pdfActionButton} onPress={handleSendEmail} disabled={pdfLoading}>
                <View style={styles.pdfActionIcon}>
              ...
              <TouchableOpacity style={styles.pdfActionButton} onPress={handlePreview}>
                <View style={styles.pdfActionIcon}>
        ...
        {project.status === 'draft' && project.notice_required && (
          <View style={styles.actionButtons}>
            <Button title="Preview & Generate Notice" onPress={handlePreview} variant="outline" fullWidth style={styles.actionButton} />
            <Button title="Send Notice" onPress={handleSend} fullWidth style={styles.actionButton} />
          </View>
        )}
...
function CameraModal({ onClose, onTakePhoto }: { onClose: () => void; onTakePhoto: (ref: CameraView | null) => void }) {
